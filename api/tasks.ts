...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      const filters: any[] = []
      if (ref_id) {
        filters.push({ property: '関連先ID', rich_text: { equals: String(ref_id) } })
      }
      if (ref_type) {
        filters.push({ property: '関連先タイプ', select: { equals: String(ref_type) } })
      }
      ...
      const tasks = response.results.map(p => toTask(p)).filter(Boolean)
